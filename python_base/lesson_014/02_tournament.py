...
from tournament import BowlingResult
import argparse

def parser_sets():
    pars = argparse.ArgumentParser(description='Calc bowling Result from File')
    pars.add_argument('input', type=str, help='Input file name with game results. The file must exists.')
    pars.add_argument('output', type=str, help='Input file name for output results')
...
if __name__ == "__main__":
    args = parser_sets()
    #test_run()
    BR = BowlingResult(args.input, args.output)
    BR.skan_file(Inlocal)
    # Усложненное задание (делать по желанию)
    BR.print_reyting()

    # example: python 02_tournament.py tournament.txt tournament_result.txt
