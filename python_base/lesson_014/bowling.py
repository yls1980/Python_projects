...
class Bowling:
    ...
    def get_score(self, fcalc_strike, fcalk_spare):
        ...
        for hit in self.game_result:
            ...
            if hit == 'X':
                ...
                calc_spare = 0
                fnc = fcalc_strike
                calc = fnc(calc, hit_number)
            elif hit == '/':
                if spare == 1:
                    spare = 0
                    count_freym += 1
                    fnc = fcalk_spare
                    calc, buf = fnc(calc, buf, hit_number)
                    calc_spare = 0
                elif spare == 0:
                    raise FirstSpeirEx(self.game_result, count_freym)
            elif hit == '-':
...
class Inlocal(Bowling):
    ...
    def get_score(self):
        return super(Inlocal, self).get_score(self.fstrike, self.fspar)


class Inmarket(Bowling):
    def fstrike(self, calc, hit_number):
        calc += 10
    ...
    def get_score(self):
        return super(Inmarket, self).get_score(self.fstrike, self.fspar)
