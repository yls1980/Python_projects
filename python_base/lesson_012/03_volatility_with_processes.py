...
def select_volatility(volatilitys_):
    vl = sorted(volatilitys_.items(), key=operator.itemgetter(1), reverse=True)
    print("Максимальная волатильность:")
    print_volat(vl)
    vl = sorted(volatilitys_.items(), key=operator.itemgetter(1))
...
class Trader(multiprocessing.Process):
    ...
    def __init__(self, file_name, volatilitys_, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.file_name = file_name
        self.volatilitys = volatilitys_
    ...
    def read_file(self):
        ...
        with open(self.file_name, 'r', encoding='cp1251') as file_f:
            try:
                ...
                self.volatilitys.put(dict(name=sname, nvol=volatility))
...
spath = 'trades'
global_volatilitys = multiprocessing.Queue()
files = [Trader(file_name=os.path.join(spath, file), volatilitys_=global_volatilitys) for file in os.listdir(spath)]
...
while not global_volatilitys.empty():
    data = global_volatilitys.get()
    volatilitys[data['name']] += data['nvol']
    # print(f"For url {data['name']} - {data['volatility']}")

select_volatility(volatilitys)
